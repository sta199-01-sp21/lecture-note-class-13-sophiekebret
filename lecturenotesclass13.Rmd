...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-sophiekebret/manhattan.csv")
```
...
```{r practice_2_ci}
ci_mice_live <- get_ci(mice_boot, level=.9)
ci_mice_live
```
